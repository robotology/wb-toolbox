...
namespace wbit {
    ...
    bool GetBiasForces::initialize(SimStruct *S, wbit::Error *error)
    {
        ...
        m_biasForces = new double[6 + dofs];
    ...
    bool GetBiasForces::output(SimStruct *S, wbit::Error *error)
    {
        ...
        if (interface) {
            ...
            interface->computeGeneralizedBiasForces(m_configuration, frame, m_jointsVelocity, m_baseVelocity, gravity, m_biasForces);

            real_T *output = ssGetOutputPortRealSignal(S, 0);
            for (unsigned i = 0; i < ssGetOutputPortWidth(S, 0); ++i) {
                output[i] = m_biasForces[i];
            }
